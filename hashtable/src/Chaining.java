...
public class Chaining {
    ...
    public int insertKey(int key){
    	...
    	//Insert key at the head of the bin. If the bin is empty, there are no other keys in the bin and no collisions.
    	//Otherwise, count the number of keys in the bin and insert key at the head of the bin.
    	if(this.Table.get(this.chain(key)).size() == 0) {
    		this.Table.get(this.chain(key)).add(0,key);
    		collision = 0;
    	} else {
    		collision = this.Table.get(this.chain(key)).size();
    		this.Table.get(this.chain(key)).add(0,key);
    	}
        return collision;
